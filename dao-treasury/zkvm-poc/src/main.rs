...
#[derive(Parser, Debug)]
#[command(about = "Verify DAO treasury zkVM settlement transcript PoC.")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    match cli.command {
        Command::Verify { transcript, output } => {
            let transcript_value = read_json_file(&transcript)?;
            let result = verify_transcript(&transcript_value)?;
...
fn verify_transcript(transcript: &Value) -> Result<Value> {
    ...
    Ok(json!({
        ...
        "tally": tally,
        "limitations": [
            "This PoC verifies transcript consistency plus CKB-native block and transaction commitments for witnessed data.",
            "Canonical CKB chain inclusion for snapshot records and vote cells is not yet proven."
        ],
    }))
}
...
fn verify_snapshot_block_witness(
    ...
    if u64_path(block, &["header", "number"])? != u64_path(public_inputs, &["snapshot_block_number"])? {
...
fn verify_snapshot_source(
    ...
    if str_path(&computed, &["record_map_root"])?
        != str_path(public_inputs, &["record_map_root"])?
    {
...
fn compute_snapshot_from_records(
    ...
    let owner_index_root =
        merkle_root_from_hex_hashes(&owner_leaf_hashes, EMPTY_OWNER_INDEX_TAG)?;
...
fn build_owner_entries(records: &[Value]) -> Result<Vec<Value>> {
    ...
    for record in records {
        ...
        object.insert("total_weight_shannons".to_owned(), json!(weight.to_string()));
        object.insert(
...
fn verify_vote_witnesses(
    ...
    for witness in witnesses {
        ...
        if str_path(commitment, &["snapshot_root"])?
            != str_path(public_inputs, &["snapshot_root"])?
        {
        ...
        let deposit_key = normalize_outpoint_key(str_path(
            commitment,
            &["deposit_out_point_key"],
        )?)?;
        ...
        if u64_path(witness, &["block_number"])?
        ...
        if u64_path(containing_block, &["header", "number"])? != u64_path(tx, &["tx_status", "block_number"])? {
            bail!("containing block number mismatch");
        }
        if str_path(containing_block, &["header", "hash"])? != str_path(tx, &["tx_status", "block_hash"])? {
        ...
        let block_transactions = get_path(containing_block, &["transactions"])?
            .as_array()
            .context("containing block transactions is not array")?;
        let block_tx = block_transactions
            .get(u64_path(witness, &["tx_index"])? as usize)
            .context("containing block tx_index out of range")?;
        if get_path(tx, &["transaction"])? != block_tx {
            bail!("transaction does not match containing block tx at tx_index");
        }
        ...
        for owner_input in owner_input_cells {
            ...
            verify_transaction_response(previous_transaction, "owner input previous_transaction witness")?;
            if str_path(previous_transaction, &["transaction", "hash"])? != str_path(owner_input, &["previous_output", "tx_hash"])? {
            ...
            if u64_path(previous_containing_block, &["header", "number"])? != u64_path(previous_transaction, &["tx_status", "block_number"])? {
                bail!("previous containing block number mismatch");
            }
            if str_path(previous_containing_block, &["header", "hash"])? != str_path(previous_transaction, &["tx_status", "block_hash"])? {
            ...
            let previous_block_txs = get_path(previous_containing_block, &["transactions"])?
                .as_array()
                .context("previous containing block transactions is not array")?;
            let previous_tx_index = u64_path(previous_transaction, &["tx_status", "tx_index"])? as usize;
            if previous_block_txs
                .get(previous_tx_index)
                .context("previous containing block tx_index out of range")?
                != get_path(previous_transaction, &["transaction"])?
            {
                bail!("previous transaction does not match containing block tx at tx_index");
            }
            ...
            let previous_output_index = u64_path(owner_input, &["previous_output", "index"])? as usize;
            let previous_outputs = get_path(previous_transaction, &["transaction", "outputs"])?
                .as_array()
                .context("previous transaction outputs is not array")?;
            let previous_outputs_data = get_path(previous_transaction, &["transaction", "outputs_data"])?
                .as_array()
...
fn json_transaction_from_response(response: &TransactionWithStatusResponse) -> Result<JsonTransactionView> {
    match response
        .transaction
        .clone()
...
fn build_tally(proposal: &Value, public_inputs: &Value, mut valid_votes: Vec<Value>) -> Result<Value> {
